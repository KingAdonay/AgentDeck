 ...
 * Icon note: resources/ sits next to out/ in dev; packaging (M5) must ship it
 * via extraResources. The @2x variant is picked up automatically for HiDPI,
 * and the 'Template' filename suffix lets macOS theme it.
 */
export class AppTray {
  private readonly tray: Tray

  constructor(private readonly handlers: TrayHandlers) {
    const icon = nativeImage.createFromPath(join(__dirname, '../../resources/trayIconTemplate.png'))
